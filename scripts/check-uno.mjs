...
let nextUid = 0;
const uid = () => `test-uid-${++nextUid}`;
const host = await call("create", { name: "Host", uid: uid() });
assert.equal(host.status, 200);
const code = host.snapshot.code;
const sessions = [{ code, token: host.token }];
for (let i = 1; i < 8; i++) {
  const p = await call("join", { code, name: `Player ${i}`, uid: uid() });
  assert.equal(p.status, 200);
  assert.equal(p.snapshot.players.length, i + 1);
  sessions.push({ code, token: p.token });
}
assert.equal((await call("join", { code, name: "Overflow", uid: uid() })).status, 400);
...
const q1 = await call("quick", { name: "Random A", uid: uid() });
const q2 = await call("quick", { name: "Random B", uid: uid() });
