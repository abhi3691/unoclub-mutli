...
async function command<T>(args: (string | number)[]): Promise<T> {
 ...
 if(!url||!token)throw new StorageError('Multiplayer storage is not configured. Add UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in Vercel and redeploy.');
 try {
  const res=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(args),cache:'no-store',signal:AbortSignal.timeout(5000)});
  const data=await res.json();
  if(!res.ok||data.error)throw new Error('Redis request failed');
  return data.result as T;
 } catch {throw new StorageError('Multiplayer storage is temporarily unavailable. Please try again.');}
}
export async function storedUnoAction(input:Input) {
 ...
 if(!configured)throw new StorageError('Multiplayer storage is not configured. Add UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in Vercel and redeploy.');
 for(let attempt=0;attempt<12;attempt++){
  ...
  if(input.action==='quick'){
   const codes=await command<string[]>(['ZREVRANGEBYSCORE',INDEX,'+inf',Date.now()-20000,'LIMIT',0,20]);
   for(const candidate of codes){
    const value=await command<string|null>(['GET',PREFIX+candidate]);
    if(!value)continue;
    const room=JSON.parse(value) as Room;
    if(room.public&&room.phase==='lobby'&&room.players.length<8){code=candidate;raw=value;store.set(code,room);action={...input,action:'join',code};break;}
   }
  ...
  const committed=await command<number>(['EVAL',COMMIT,2,PREFIX+code,INDEX,raw??'',room?JSON.stringify(room):'',code!,room?.public&&room.phase==='lobby'&&room.players.length<8?'1':'0',Date.now(),Date.now()-1800000]);
