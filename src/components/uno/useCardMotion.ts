...
type Captured = { snapshot: Snapshot; cards: Map<string, { box: Position; face: HTMLElement }> };
...
export function useCardMotion(snapshot: Snapshot | null) {
  ...
  useLayoutEffect(() => {
    const cancel = () => { for (const finish of [...active.current]) finish(); };
  ...
  useLayoutEffect(() => {
    ...
    hand.querySelectorAll<HTMLElement>("[data-card-id]").forEach(element => {
      cards.set(element.dataset.cardId!, { box: position(element), face: element.cloneNode(true) as HTMLElement });
    });
    previous.current = { snapshot, cards };
    if (!old || old.snapshot.code !== snapshot.code || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const seat = (id: string) => document.querySelector<HTMLElement>(`[data-player-id="${id}"] .avatar`);
    const fly = (face: HTMLElement, from: Position, to: Position, delay = 0, landing?: HTMLElement, drawing = false) => {
      ...
      const animation = clone.animate([
        ...
        { transform: `translate(${dx * .48}px,${dy * .48 - 45}px) rotate(${drawing ? -12 : 12}deg) scale(1.08)`, offset: .48 },
    ...
    if (topChanged && old.snapshot.phase === "playing") {
      ...
      const source = owned?.box ?? (seat(old.snapshot.turn) ? position(seat(old.snapshot.turn)!) : null);
      if (source) fly(owned?.face ?? discard, source, position(discard), 0, discard);
    }
    const added = snapshot.hand.filter(card => !old.snapshot.hand.some(previousCard => previousCard.id === card.id));
    ...
    added.forEach((card, index) => {
      ...
      const visible = target.left >= handBox.left && target.left + target.width <= handBox.left + handBox.width;
      const destination = { ...target, left: Math.max(handBox.left + 6, Math.min(target.left, handBox.left + handBox.width - target.width - 6)) };
      fly(pile, position(pile), destination, index * 75 + (topChanged ? 180 : 0), visible ? element : undefined, true);
    });
    for (const player of snapshot.players) {
      ...
      if (gained > 0 && target) for (let i = 0; i < Math.min(gained, 7); i++) {
        fly(pile, position(pile), position(target), i * 75 + (topChanged ? 180 : 0), undefined, true);
      }
    }
  }, [snapshot]);
}
