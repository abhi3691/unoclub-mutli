import { z } from 'zod';
import { unoAction } from '@/uno/server';
...
export async function POST(request:Request){
 ...
 try{const body=await request.text();if(body.length>20000)throw new Error('Request too large');const result=unoAction(schema.parse(JSON.parse(body)));return Response.json(result,{headers:{'Cache-Control':'no-store'}});}catch(e){return Response.json({error:e instanceof z.ZodError?'Invalid request':e instanceof Error?e.message:'Request failed'},{status:400});}
}
